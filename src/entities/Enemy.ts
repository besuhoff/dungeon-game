...
export class Enemy extends ScreenObject implements IEnemy {
  ...
  shoot(dt: number): void {
    ...
    if (this.world.multiplayerMode === "host") {
  ...
  update(dt: number): void {
    ...
    if (player && this.canSeePlayer()) {
      this._rotation =
        90 + (Math.atan2(this.y - player.y, this.x - player.x) * 180) / Math.PI;
      // Shoot at player
      this.shoot(dt);
      return;
    }
    ...
    if (!collision) {
      for (const enemy of this.world.enemies.filter(
        (enemy) => enemy.isAlive() && enemy.id !== this.id
      )) {
        if (
    ...
    if (this._wall.orientation === "vertical") {
      if (y < this._wall.y || y > this._wall.y + this._wall.height) {
        ...
        y = Math.max(
          this._wall.y,
          Math.min(this._wall.y + this._wall.height, y)
        );
      }
    } else {
      if (x < this._wall.x || x > this._wall.x + this._wall.width) {
        ...
        x = Math.max(
          this._wall.x,
          Math.min(this._wall.x + this._wall.width, x)
        );
      }
    }
    if (this._wall.orientation == "vertical") {
